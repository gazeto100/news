...
mycursor.execute("SELECT title FROM dnesbg WHERE site = 'dariknews.bg' ORDER BY id DESC LIMIT 1000")
...
for x in range(len(getRealNews)):
    ...
    val = (getRealNews[x].strip(), " ", "http:"+getRealNewsLink[x], "http:"+getRealNewsImg[x], "dariknews.bg", time)
