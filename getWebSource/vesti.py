...
mycursor.execute("SELECT title FROM dnesbg WHERE site = 'vesti.bg' ORDER BY id DESC LIMIT 1000")
...
for x in range(len(getRealNews)):
    ...
    val = (getRealNews[x], " ", getRealNewsLink[x], getRealNewsImg[x], "vesti.bg", time)
